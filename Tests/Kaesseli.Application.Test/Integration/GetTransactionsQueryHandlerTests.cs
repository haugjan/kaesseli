...
public class GetTransactionsQueryHandlerTests
{
    ...
    public async Task Handle_ReturnsCorrectTransactions()
    {
        ...
        var transactionSummeryGuid = Guid.NewGuid();
        var transactions1 = new SmartFaker<Transaction>()
                           .RuleFor(tran => tran.TransactionSummary, 
                                    value: new SmartFaker<TransactionSummary>()
                                        .RuleFor(ts=> ts.Id, transactionSummeryGuid ).Generate())
                           .Generate(count: 5);
        ...
        mockRepository.Setup(repo => repo.GetTransactions(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
                      .ReturnsAsync(
                          (Guid transactionSummaryId, CancellationToken _)
                              => transactions1.Concat(transactions2).Where(tran => tran.TransactionSummary!.Id == transactionSummaryId));

        var handler = new GetTransactionsQueryHandler(mockRepository.Object);
        var query = new GetTransactionsQuery { TransactionSummaryId = transactionSummeryGuid };
        ...
        result.Should()
              .BeEquivalentTo(transactions1, options=> options.Excluding(t=> t.TransactionSummary));

        mockRepository.Verify(
            repo => repo.GetTransactions(transactionSummeryGuid, It.IsAny<CancellationToken>()),
            Times.Once);
    }
}