...
public static class GetNextOpenTransaction
{
    ...
    {
        Task<Contracts.Integration.OpenTransaction?> Handle(
            Query request,
    ...
    ) : IHandler
    {
        ...
        {
            var transaction = await transactionRepository.GetNextOpenTransaction(
                request.Skip,
                cancellationToken
            );
            ...
            return new Contracts.Integration.OpenTransaction(
                ...
                SuggestedAccounts: accounts.Select(
                    account => new Contracts.Integration.SuggestedAccount(
                        Relevance: 0,
